...
use std::{env};
use std::io::{Read, BufWriter, Write, Seek};
...
fn main() {
    ...
    if let Ok(opened_file) =  OpenOptions::new().write(true).read(true).open(&save_as) {
        ...
        file =  File::create(save_as.clone()).expect(&format!("Could not create file '{}'", save_as.clone()));
    }
    let total_size = reqwest::get(site.get_video_url().as_str()).expect("Could not make request to get size").content_length().expect("Could not get content length");
    let mut cursor = 0;
    ...
    if should_continue.eq("true"){
        ...
        writer.seek(Start(cursor)).expect("Could not seek to continue download");
